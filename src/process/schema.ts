...
import {parseSelectorTarget} from '../utility/annotations';
...
const annotation = new schema.Entity('annotations', {}, {
    processStrategy: (original: any): any => {
        const body = Array.isArray(original.body) ? original.body : [original.body];
        const target = Array.isArray(original.target) ? original.target : [original.target];
...
const annotationBody = new schema.Union(
    ...
    (input: any): 'choice' | 'contentResource' => {
